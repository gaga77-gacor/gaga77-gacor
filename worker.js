  ...
  async fetch(request, env) {
    ...
    try {
      ...
      /* ME */
              ...
              /* =====================================================
           GAME BALANCE ADJUST
           D1 = authoritative balance source
           delta < 0 = debit taruhan
           delta > 0 = payout
           ===================================================== */
...
async function ensureGameSettingsTable(env) {
  ...
  await env.DB.prepare(`
    CREATE TABLE IF NOT EXISTS game_settings (
      ...
      rtp REAL NOT NULL DEFAULT 95,
      max_payout REAL NOT NULL DEFAULT 10000000,
      updated_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP
    )
  `).run();
...
const DEFAULT_GAME_SETTINGS = [
  ...
  ["baccarat_idn_1", "Baccarat IDN 1"],
  ["baccarat_idn_2", "Baccarat IDN 2"],
  ["baccarat_idn_3", "Baccarat IDN 3"],
  ["spaceman", "Spaceman"]
];


async function seedDefaultGameSettings(env) {

  await ensureGameSettingsTable(env);

  for (
    const [gameId, gameName]
    of DEFAULT_GAME_SETTINGS
  ) {
...
async function getAllGameSettings(env) {

  await seedDefaultGameSettings(env);
  ...
  return (
    ...
    updatedAt:
      row.updated_at
        ...
        /* =====================================================
           ADMIN GAME SETTINGS - GLOBAL
           Berlaku sama untuk seluruh game.
        ===================================================== */

        if (
          path === "/admin/game-settings/global" &&
          request.method === "POST"
        ) {
          ...
          const maxPayout =
            Number(body?.maxPayout);

          const applyRtp =
            body?.applyRtp !== false;

          const applyMaxPayout =
            body?.applyMaxPayout !== false;

          const applyEnabled =
            body?.applyEnabled === true;
          ...
          if (
            ...
            return json({
              success: false,
              message:
                "RTP Global harus antara 1 sampai 100."
            }, 400);
          ...
          return json({
            success: true,
            message:
              "Pengaturan global berhasil diterapkan.",
            updated
          });
...
if (
          ...
          for (const [gameId, gameName] of defaults) {
            await env.DB.prepare(`
              INSERT OR IGNORE INTO game_settings
              (game_id, game_name, enabled, rtp, max_payout)
              VALUES (?, ?, 1, 95, 10000000)
            `)
            .bind(gameId, gameName)
            .run();
          }
          ...
          return json({
            success: true,
            games: (result.results || []).map(row => ({
              gameId: row.game_id,
              gameName: row.game_name,
              enabled: Number(row.enabled) === 1,
              rtp: safeNumber(row.rtp),
              maxPayout: safeNumber(row.max_payout),
        ...
        if (
          ...
          if (
            ...
            return json({
              success: false,
              message: "RTP harus antara 1 sampai 100."
            }, 400);
          }
          ...
          if (
            ...
            return json({
              success: false,
              message: "Max payout tidak valid."
            }, 400);
          }
          ...
          if (
            ...
            return json({
              success: false,
              message: "Pengaturan game gagal disimpan."
            }, 500);
          }
          ...
          return json({
            ...
            game: {
              ...
              enabled: Number(game.enabled) === 1,
              rtp: safeNumber(game.rtp),
              maxPayout: safeNumber(game.max_payout),
...
if (
          ...
          if (
            ...
            return json(
              {
                success: false,
                message: "Delta saldo tidak valid."
              },
              400
            );
          }
          ...
          return json({
            ...
            message:
              delta < 0
                ? "Saldo taruhan berhasil dikurangi."
                : "Payout berhasil ditambahkan.",
            game: gameName,
            delta,
            saldo,
            user: {
